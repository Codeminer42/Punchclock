module Users
  class RegistrationsController < Devise::RegistrationsController
    ...
    def create
      ...
      if resource.save
        if resource.active_for_authentication? then signed_success
        else signed_inactive_response
        end
      else
    ...
    def signed_success # MOVEME to an responder
      set_flash_message :notice, :signed_up if is_navigational_format?
      sign_up(resource_name, resource)
      respond_with resource, location: after_sign_up_path_for(resource)
    end

    def signed_inactive_response # MOVEME to an responder
      set_flash_message(
        :notice,
