...
ActiveAdmin.register Punch do
  ...
  filter :project, collection: -> { User.projects.order(:name) }
  ...
  form do |f|
    f.inputs do
      f.input :user, as: :select, collection: option_groups_from_collection_for_select(:users, :name, :id, :name, f.object.user_id)
      f.input :project, as: :select, collection: option_groups_from_collection_for_select(:projects, :name, :id, :name, f.object.project_id)
