...
describe ActiveAdmin::StatsHelper do
  describe '#constributions_offices_data' do
    ...
    let(:user_no_contributions) { create(:user, company: company, office: office) }

    let!(:contribution) { create(:contribution, :approved, user: user, company: company) }

    describe 'returned hash' do
      let!(:data) do
        helper.constributions_offices_data(ContributionsByOfficeQuery.new.by_company(company).approved.to_relation)
      end
    ...
    describe '#constributions_users_data' do
      let!(:data) do
        contribs = ContributionsByUserQuery.new.by_company(company).approved.to_hash
